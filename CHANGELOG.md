...
- `IGitRepository.pushBranch(repo, branchName)` + `GitRepositoryBranches.pushBranchImpl`: явный `RefSpec("refs/heads/x:refs/heads/x")`, в отличие от обычного `push()`, который через дефолтное поведение JGit всегда бьёт по HEAD. Позволяет запушить любую локальную ветку без переключения на неё; резолв credentials и обработка сетевых ошибок — как у обычного push.
- `BranchesViewModel.pushBranch(branch)`; пункт **«Push branch»** в контекстном меню (⋮) — доступен для любой локальной ветки, включая текущую (как Rename), без диалога подтверждения (push не разрушает локальное состояние).
...
- Иконка «Детали» в компактной панели коммита красится цветом `primary` (как текст «Детали» в раскрытом виде).
- Диалог веток: убрано дублирующее сообщение о конфликте (`BranchesViewModel.conflict()` больше не пишет errorMessage — баннер операции и так всё показывает); при открытии диалога стираются устаревшие message/error (после отмены из хэдера сообщение больше не висит).
- Кнопки выбора версии в конфликте переверстаны в единый вертикальный layout (одинаковая ширина, ничего не съезжает); подписи — с именами веток: «Current branch (main)» / «Incoming branch (feature)».
- Подписи сторон в side-by-side диффе конфликтного файла синхронизированы: вместо Old/New — те же «Current branch (…)» / «Incoming branch (…)».
- **Учтена инверсия ours/theirs при rebase:** метки «current/incoming» вычисляются с поправкой на то, что во время rebase текущая ветка — перебазируемая (source), а не onto. Так подписи совпадают с реальным действием кнопок (флип стратегии уже был).
- Кнопка удаления ветки перенесена из строки в контекстное меню (⋮) к Merge/Rebase — отдельным пунктом под разделителем, error-цветом; для локальных веток.
- Исправлена вёрстка карточки GitLab в разделе аккаунтов: колонка кнопок (GitLab.com/Self-hosted) не ограничивалась по ширине и в `Row` сжимала статус «Не подключен» до одного символа в строке. Колонка ограничена `132.dp`. Остальные карточки экрана проверены — паттерн больше не встречается.

## 2026-07-04 (5)

### Added — контекст конфликта «что → куда»
- Новая модель `ConflictInfo` (source/target ветки + involved-коммиты + пути) и метод `getConflictInfo` в `IGitRepository`: для merge берёт target = текущая ветка, source = из `MERGE_MSG`/`MERGE_HEAD`; для rebase читает `.git/rebase-merge|apply/{head-name,onto}`.
- Попап конфликта в хэдере теперь показывает строку «Слияние "X" в "Y"» / «Rebase "X" на abc1234» над списком файлов.
- В графе два коммита конфликтующей операции (HEAD + MERGE_HEAD / onto) обводятся **красным пунктиром** (`Modifier.dashedBorder`). Хэши прокидываются из синхронизированного `conflictInfo`, отдельной загрузки в графе нет.

### Changed
- Кнопка «Детали» переехала в строку с иконками действий (компакт-режим) — больше не занимает отдельную строку, высота панели в свёрнутом виде уменьшилась. В режиме деталей — кнопка сворачивания снизу.
- `MainViewModel`: `operationState`+`conflictPaths` объединены в единый `conflictInfo`.
