...
- `FileOperations` - stage, unstage, restore
- `TagOperations` - теги

**Рекомендация для `CommitDetailDialog.kt`** - вынести:
- `CommitDetailViewModel` - логика загрузки данных
- `DiffViewer` - отдельный компонент для просмотра diff
- `FileTreeViewer` - дерево файлов коммита
- `CommitInfoCard` - карточка с метаданными коммита
...
### 3.5 Нет ViewModel для основных экранов
- **Файлы:** `MainScreen.kt`, `ChangesScreen.kt`
- **Серьёзность:** СРЕДНЯЯ
...
```kotlin
// Текущий код - загружает ВСЁ:
for (commit in revWalk) {
    commits.add(...)  // Бесконечный рост списка
}
```
...
**Рекомендация:** Загрузить все теги один раз перед обходом коммитов, построить HashMap<commitHash, List<String>>.
...
**Рекомендация:** Использовать `use { }` или `try-finally`.
...
### 5.4 Несогласованная обработка ошибок
- **Описание:** Используется микс подходов:
  - `Result<T>` (stage, commit, push, pull)
  - Возврат `null` (openRepository, getFileContent)
  - Возврат `emptyList()` (getCommits, getBranches)
  - Выброс исключений (AuthManager)
  - `Boolean` (restoreFile)

**Рекомендация:** Стандартизировать на sealed class:
```kotlin
sealed class GitResult<out T> {
    data class Success<T>(val data: T) : GitResult<T>()
    data class Error(val exception: Throwable, val message: String) : GitResult<Nothing>()
}
```
...
### Фаза 1: Безопасность и стабильность (MUST HAVE для релиза)

- [ ] Заменить SharedPreferences на EncryptedSharedPreferences
- [ ] Удалить все логи с секретами/токенами (179 вызовов Log)
- [ ] Заменить встраивание токенов в URL на CredentialsProvider
- [ ] Добавить PKCE в OAuth-поток
- [ ] Добавить валидацию URL в WebView
- [ ] Исправить force unwrap (`!!`) на безопасные вызовы
- [ ] Добавить ProGuard-правила для JGit, Retrofit, Gson
- [ ] Включить `isMinifyEnabled = true` для release
...
- [ ] Оптимизировать загрузку тегов (однократная загрузка)
- [ ] Исправить утечки ресурсов (use { } для JGit-объектов)
- [ ] Стандартизировать обработку ошибок

### Фаза 3: Тестирование и CI/CD

- [ ] Добавить unit-тесты для AuthManager
- [ ] Добавить unit-тесты для Git-операций
...
Тестов:                      3
TODO/FIXME:                  4
Файлов без интерфейсов:      Все
Dependency Injection:        Нет
CI/CD:                       Нет
